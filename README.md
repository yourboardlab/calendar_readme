...
필수 가입은 없습니다. 설정 맨 아래 **「로그인 기능 사용」** 을 켜면:

- **가입:** 이메일 입력 → **6자리 인증 코드**(메일, **5분** 유효) → 비밀번호 등록(영문·숫자·특수문자, 8자 이상)
- **실행 시:** 등록한 이메일·비밀번호로 잠금 해제 후 달력 사용
- **비밀번호 재설정:** 로그인 화면에서 기존 비밀번호 폐기 후, 같은 이메일로 인증·새 비밀번호 등록
- **비밀번호 표시:** 입력 칸 옆 **눈 아이콘**으로 보기/숨기기
- **저장:** 비밀번호는 기기 안에 **암호화(단방향 해시)** 로만 저장되며, 평문은 남지 않습니다.

인증 메일 발송만 [Vercel `dailylog` 프로젝트](https://vercel.com/yourboardlab-s-projects/dailylog)의 서버리스 API를 사용합니다. **일기·가계부 본문은 서버로 보내지 않습니다.**

<p align="center">
  <img src="assets/release_drawer_settings.png" alt="설정 화면 (언어·보내기·로그인)" width="300" />
</p>
...
- **일기·가계부·체크·기록·기분**은 **휴대폰 안에만** 저장됩니다. 클라우드 동기화·광고 추적은 없습니다.
- **로그인**은 선택 사항입니다. 켜 두면 이메일·비밀번호 **해시**가 기기에 저장되고, 인증 코드 메일 발송 시에만 네트워크를 씁니다.
- 백업: **설정 → 데이터보내기** 로 **ZIP(v4)** 하나에 **달력 데이터 + 앱 설정 + 로그인(켜져 있을 때)** 를 담습니다. 새 기기에서 **데이터 불러오기**로 복원할 수 있습니다.
- 입력 내용은 **항상 자동 저장**됩니다(별도 저장 버튼 없음).
